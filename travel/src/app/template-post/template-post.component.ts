...
export class TemplatePostComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.suggestion != undefined) {
      console.log('sug-dates!', this.suggestionBookedDates)
      console.log('sug-mapper', this.suggestionMapper)
      this.suggestion.bookedDates = this.suggestionBookedDates;
      this.suggestion.mapperServices = this.suggestionMapper;
      console.log('after init', this.suggestion);
      ...
      this.form = new FormGroup({
        ...
        toppings: new FormControl(this.suggestion.mapperServices, Validators.required),
      });
      ...
      if (this.suggestion.bookedDates != undefined) {
        if (this.suggestion.bookedDates.length > 0 && this.suggestion.bookedDates.length > 1) {
          ...
          console.log('bookedDAte---', this.suggestion.bookedDates[0].dateStart);
          console.log('bookedDAte---', this.suggestion.bookedDates[0].dateEnd);
  ...
  @Input() suggestionBookedDates: bookedDate[];
  @Input() suggestionMapper: ServiceTypeMapper[];
  ...
  getErrorMessageTypeOf() {
    if (this._toppings.hasError('required')) {
      return 'You must enter a value';
    }
    return this._toppings.invalid ? 'Not a valid street' : '';
  }
  x = 0;

  addDates(newDates: bookedDate[]) {
    console.log('templatr get dates: !!!!!!!!!!!!1', newDates);
  ...
  onSubmit() {
    ...
    if (this.form.get('title').value && this.form.get('country').value && this.form.get('city').value && this.form.get('street').value) {
      ...
      if (this.range.get('start').value != null) {
        ...
        console.log(this.new_post, "wwith datessss");
      ...
      //this.new_post.ServicesTypeName = this.toppings.value.map((v: ServiceTypeMapper) => { return v.ServiceTypeName });
      this.new_post.mapperServices = this.form.get('toppings').value;
      ...
      if (this.status) {
        ...
        console.log('daiiii', this.new_post);
        this.currentUser = JSON.parse(localStorage.getItem("currentUser"));
        this.new_post.HostId = this.currentUser.UserID;
